...
// ARMO EL ARRAY CON LOS DATOS DE LOS RANGOS DE GASTOS
$array_sec = array();
$array_min = array();
$array_max = array();
$array_monto = array();
$i = 0;
while($row_rangos = mysqli_fetch_array($rangos)) {
	$array_sec[$i] = $row_rangos["secuencia"];
	$array_min[$i] = $row_rangos["monto_min"];
	$array_max[$i] = $row_rangos["monto_max"];
	$array_monto[$i] = $row_rangos["monto_fijo"];
	$i++;
}
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
    ...
    $ImpTotConc   = 0.00;
    $ImpNeto      = $_POST['totneto21'] + $_POST['totneto105'] + $_POST['totcomis'] + $_POST['totimp'];
    $ImpOpEx      = 0.00;
	...
	if ($ta = $wsaa->loginCms())
	{
    	 ...
    	 */
		if (isset($CompUltimoAutorizado['CbteNro']))
    		$CbteDesde = $CompUltimoAutorizado['CbteNro'] + 1;
		else
			$CbteDesde = 1;
		//echo "CbteDesde = ".$CbteDesde."  -";
    	$CbteHasta = $CbteDesde;
        ...
        */
       	...
       	if (!$FeCAEResponse)   	{
           	...
           	echo '<h2 class="err">NO SE HA GENERADO EL CAE</h2>
               	  <h2 class="err">ERRORES DETECTADOS</h2>';
       	...
       	elseif (!$FeCAEResponse['FeDetResp']['FECAEDetResponse']['CAE']) {
           	...
           	echo '<h2 class="msg">NO SE HA GENERADO EL CAE</h2>
               	  <h2 class="msg">OBSERVACIONES INFORMADAS</h2>';
...
if ($sigo_y_grabo == 1 && $todo_ok ==1) {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			...
			//============================================================================
  			$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp, nreng, codrem, codlote, descrip, neto, porciva, comcob, usuario) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       FC_A2, //GetSQLValueString($_POST['tcomp'], "int"),
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			...
			$renglon = 2;
  			$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp, nreng, codrem, codlote, descrip, neto, porciva, comcob, usuario) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s)",
					   ...
					   $cod_usuario); //GetSQLValueString($_POST['codusu'], "int"));
	...
	if (isset($_POST['lote']) && GetSQLValueString($_POST['lote'], "int")!="NULL") {
  		...
  		$query_mascara = "SELECT * FROM series  WHERE  series.codnum='52'";//'$tcomp'  AND series.codnum='$serie'";
	...
	$query_facturas_a = "SELECT * FROM series  WHERE series.tipcomp= $tipcomp  AND series.codnum= $numserie";
	$facturas_a = mysqli_query($amercado, $query_facturas_a) or die("ERROR LEYENDO TABLA SERIES");
...
var selec = form.tipos.options;
		...
		factura.serie_texto.value = "SERIE DE FACTURA A0002";
		factura.tcomp.value = 115;
...
$impuesto->data_seek(0);
$row = $impuesto->fetch_array();
$iva_21_desc = $row[2]."<br>";
$iva_21_porcen = $row[1];

$impuesto->data_seek(1);
$row = $impuesto->fetch_array();
$iva_15_desc = $row[2]."<br>";
$iva_15_porcen = $row[1];
?>
<meta http-equiv="content-type" content="text/html; charset=UTF-8"/>
...
function getprov(form) {
	...
	if (total === form.lote1.value || total === form.lote2.value || total === form.lote3.value || total === form.lote4.value || total === form.lote5.value || total === form.lote6.value || total === form.lote7.value || total === form.lote8.value || total === form.lote9.value || total === form.lote10.value || total === form.lote11.value || total === form.lote12.value || total === form.lote13.value || total === form.lote14.value) { 
		alertaError("EL lote ya ha sido seleccionado");
		form.lote.value = "" ;
...
<script language="javascript">
function calc_gastos(form)
{
	var tot_tasa = 0;
	var neto = 0.00;
	if (factura.tasa.value!=0) {
		neto = factura.importe.value;
		if (neto <= 5000)
			tot_tasa = 100.0 * (factura.tasa.value / 100.0);
		else if(neto <= 20000)
				tot_tasa = 500.0 * (factura.tasa.value / 100.0);
		else if (neto <= 40000)
			tot_tasa =  1200.0 * (factura.tasa.value / 100.0);
		else if (neto <= 80000)
			tot_tasa = 2400.0 * (factura.tasa.value / 100.0);
		else if (neto <= 100000)
			tot_tasa =  3600.0 * (factura.tasa.value / 100.0);
		else if (neto <= 150000)
			tot_tasa = 5000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 200000)
			tot_tasa =  7000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 300000)
			tot_tasa = 10000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 400000)
			tot_tasa = 14000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 500000)
			tot_tasa = 18000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 600000)
			tot_tasa = 22000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 700000)
			tot_tasa = 26000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 800000)
			tot_tasa = 30000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 1000000)
			tot_tasa = 36000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 1500000)
			tot_tasa = 50000.0 * (factura.tasa.value / 100.0);
		else if (neto <= 2000000)
			tot_tasa = 70000.0 * (factura.tasa.value / 100.0);
		else
			tot_tasa = 80000.0 * (factura.tasa.value / 100.0);
		
		tot_tasa=tot_tasa.toFixed(2);
		factura.totimp.value = tot_tasa;
		factura.tot_general.value = tot_neto + tot_tasa;
	}
	else {
...
<script type="text/javascript">
	...
	{
		//alertaError("Estoy en validar_factura 1 ");
    	...
    	if (imp==0) {
	  		alertaError("Falta un tipo de impuesto");
  		} else {
			//alertaError("Voy al submit ");
	  		form.submit()
			//alertaError("Pase el submit ");
  		}
	}
	</script>
    ...
    <tr>
      ...
      <td width="14%" height="20" bgcolor="#FFFFFF"><span class="ewTableHeader">Tipo de Comprobante </span></td>
          <td width="40%"><select name="tipos" onchange="agregarOpciones(this.form)">
                                      <option value="115">FACTURA POR LOTES A0002</option>
                                     
                           </select>
		  <input name="tcomp" id="tcomp"  type="hidden" value="115"/>
		         </td>
          <td width="1%">&nbsp;</td>
          <td width="12%" class="ewTableHeader">Serie</td>
          <td width="33%"><input name="serie_texto" type="text"  size="25" value="SERIE DE FACTURA A0002"/>
		  <input name="serie" type="hidden"  size="25" value="52"/>
            </td>
        </tr>
        <tr>
    ...
    <tr>
	  ...
	  <table width="100%" border="1" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
        <tr>
          ...
          <td width="58" rowspan="2" class="ewTableHeader"  background="images/fonod_lote.jpg"> <div align="center">% GsAdm</div></td>
      ...
      do {  
      ?>
            <option value="<?php if (isset($row_lotes_rem['codintlote'])) echo $row_lotes_rem['codintlote'];?>"><?php if (isset($row_lotes_rem['codintlote']))  echo $row_lotes_rem['codintlote']; ?><?php echo " "?><?php if (isset($row_lotes_rem['codintlote']))  echo substr($row_lotes_rem['descor'],0,49); ?><?php echo " | "?><?php if (isset($row_lotes_rem['codintlote']))  echo $row_lotes_rem['comiscobr']; ?><?php echo " | "?><?php if (isset($row_lotes_rem['codintlote']))  echo 100; ?><?php echo " | "?><?php if (isset($row_lotes_rem['codintlote']))  echo $row_lotes_rem['preciobase']; ?></option>
            <?php
    ...
    <tr>
      <td colspan="3" bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
        <tr>
          ...
          <td height="20" background="images/fonod_lote.jpg" class="ewTableHeader"><div align="center">Uso Plataf </div></td>
          ...
          <td height="20" background="images/fonod_lote.jpg" class="ewTableHeader"><div align="center">Gs. adm</div></td>
