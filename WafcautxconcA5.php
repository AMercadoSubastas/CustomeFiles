		...
		if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
			...
			if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
  				$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp, nreng, codrem , descrip, neto,  concafac, usuario) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
					   ...
					   1, //GetSQLValueString($_POST['secuencia'], "int"),
	...
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			...
			if (!empty($_POST['imprime'])) { 
				...
				$numserie = SERIE_A5;  //GetSQLValueString($_POST['serie'], "int");
...
mysqli_select_db($amercado, $database_amercado);
$query_facturas_a = "SELECT * FROM series  WHERE series.codnum=54"; // antes decia 1 que es por lotes
...
$tcomp = 127;
mysqli_select_db($amercado, $database_amercado);
$query_facturas_b = "SELECT * FROM series  WHERE series.tipcomp=127"; // antes decia 23 que es por lotes
...
mysqli_select_db($amercado, $database_amercado);
$query_tipo_comprobante = "SELECT * FROM tipcomp WHERE codnum='127'";
...
function ValidoForm(form)
{
    ...
    var cliente = form.cliente.value; // Codigo de cliente
	 ...
	 //alertaError("imp ="+tipoiva);
	 //alertaError("usuario ="+usuario);
...
function agregarOpciones(form)
{
	var selec = form.tipos.options;
		...
		factura.serie_texto.value = "SERIE DE FC CONC MiPyme A0005";
		factura.tcomp.value = 127;
...
<body>
<form id="factura" name="factura" class="container" method="POST" action="VWafcautxconcA5">
  ...
  <table  border="1" align="left" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF" width="900">
    ...
    <tr>
      <td colspan="4" valign="top" bgcolor="#FFFFFF"><table width="100%" border="1" cellspacing="1" cellpadding="1">
		 ...
		 <td width="16%" height="20" class="ewTableHeader">Tipo de Comprobante </td>
          <td width="35%"><select name="tipos" onchange="agregarOpciones(this.form)">
                        <option value="127">FC CONCEPTOS A0005 MiPyme</option>
                                     
                           </select>
		  <input name="tcomp" type="hidden" value="127" size="25" >
		         </td>
          
          <td width="14%" class="ewTableHeader">Serie</td>
          <td width="35%"><input name="serie_texto" value="SERIE DE FC CONC A0005 MiPyme" type="text"  size="44" />
...
?>
			...
			<td bgcolor="#FFFFFF"><input name="tipoiva" type="text" size="10" required="required" onchange="getprov(this.form)">
		  </td>
