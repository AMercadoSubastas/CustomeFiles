...
mysqli_select_db($amercado, $database_amercado);
   ...
   // if ($ptcomp == 51 || $ptcomp == 52 || $ptcomp == 55 || $ptcomp == 57 || $ptcomp == 59 || $ptcomp == 61 || $ptcomp == 63)
	...
	if (isset($codp_cliente))
		$pdf->Cell(70,10,$calle_cliente.' '.$nro_cliente.' ('.$codp_cliente.') '.utf8_decode($nomloc),0,0,'L');
	else
			$pdf->Cell(70,10,$calle_cliente.' '.$nro_cliente.'     '.utf8_decode($nomloc),0,0,'L');
	// Datos del Remate
    $pdf->SetFont('Arial','',9);
	//$pdf->Ln(9);
	...
	if ($remate!="" && $remate!=0) {
		...
		$pdf->Cell(30,10,$remate_direc." ,".$nomlocrem." ,".utf8_decode($nomprovrem),0,0,'L');
		//$pdf->Cell(130);
 		$pdf->SetX(15);
        $pdf->SetY(85);
        $pdf->SetFont('Arial','BI',10);
        ...
        $pdf->Cell(2);
		if ($pncomp == 306)
			$pdf->Cell(15,10,$remate." Por cuenta y orden de terceros.",0,0,'L');
		else
	...
	if ($ptcomp == 115 || $ptcomp == 117) {
      ...
      $pdf->MultiCell(12,7,$df_codintlote,0,'L');
	}
	$pdf->SetY($j);
	$pdf->SetX(27);
	...
	if (isset($df_descrip3)) {
		$pdf->SetX(27);
		$pdf->MultiCell(150,7,$df_descrip1,0,'L');
		$pdf->SetY($j+3);
		$pdf->SetX(27);
		$pdf->MultiCell(150,7,$df_descrip2,0,'L');
		$pdf->SetY($j+6);
		$pdf->SetX(27);
		$pdf->MultiCell(150,7,$df_descrip3,0,'L');
		$pdf->SetY($j);
        $df_descrip1 = "";
		$df_descrip2 = "";
        $df_descrip3 = "";
	...
	if (isset($df_descrip2)) {
		...
		$pdf->MultiCell(150,7,utf8_decode($df_descrip1),0,'L');
		$pdf->SetY($j);
		$df_descrip1 = "";
		$df_descrip2 = "";
	}
	$pdf->SetX(158);
	$pdf->MultiCell(44,7,$df_importe,0,'R');
    ...
    if ($ptcomp != 117 && $ptcomp != 121 && $ptcomp != 127) {
        ...
        if ($pserie==52 ) {	
            ...
            $pdf->Cell(0,8,utf8_decode('  Neto 0 %                  Neto 10,5 %            Uso Plataforma                   IVA 21 %                        IVA 10,5 %                  Gs  Adm.            Importe Total'),0,0,'L');
        ...
        $pdf->SetY(-50);
        $pdf->SetX(10);
        $pdf->Cell(188,3,utf8_decode("Luego de su aceptación tácita o expresa, esta Factura de Crédito Electrónica MiPyMEs será transmitida a El Sistema de Circulación Abierta para Facturas de Crédito Electrónicas MiPyMEs, para su"),0,0,'C');
        $pdf->SetY(-47);
        $pdf->SetX(10);
        $pdf->Cell(188,3,utf8_decode("circulación y negociación incluso en los Mercados de Valores, en este caso a través de un Agente de Depósito Colectivo o agentes que cumplan similares funciones"),0,0,'C');
        $pdf->SetY(-72);
        $pdf->SetFont('Arial','I',8);
        if ($pserie==52 || $pserie == 54) {	
            ...
            // FActura A
            if ($ptcomp == 119 || ($ptcomp == 122 && $tot_iva21 == 0.00 && $tot_iva105 == 0.00))
            $pdf->Cell(0,8,utf8_decode('  Neto 0 %                  Neto 10,5 %             Serv. prestados                   IVA 21 %                        IVA 10,5 %                  Tasa Adm.            Importe Total'),0,0,'L');
            else if ($ptcomp == 125 && $tot_neto0 != 0)
                $pdf->Cell(0,8,utf8_decode('  Neto 21 %                  Neto 0 %             Serv. prestados                   IVA 21 %                        IVA 10,5 %               Tasa Adm.            Importe Total'),0,0,'L');
            else
                $pdf->Cell(0,8,utf8_decode('  Neto 21 %                  Neto 10,5 %             Serv. prestados                   IVA 21 %                        IVA 10,5 %               Tasa Adm.            Importe Total'),0,0,'L');
