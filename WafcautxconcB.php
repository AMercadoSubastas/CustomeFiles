...
*/
	...
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {	
			...
			$insertSQL = sprintf("INSERT INTO cabfac (tcomp, serie, ncomp, fecval, fecdoc, fecreg, cliente, fecvenc, estado, emitido, codrem, totbruto, totiva21, totneto21, nrengs, nrodoc, en_liquid, CAE, CAEFchVto, Resultado, usuario, usuarioultmod) VALUES (%s, %s, %s, '$fecha_factura1','$fecha_factura1', '$fecha_factura1', %s, '$fecha_factura1', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						   FC_B2,
                       	   SERIE_B2,
						   $num_fac, //GetSQLValueString($_POST['num_factura'], "int"),
	...
	for ( contador ; contador < cant ; contador++) {
    	...
    	if (seleccion[contador].selected) { 
			...
			if (pos1 !== -1) {
				...
				if (pos2 !== -1) {
					lugar = lugar.substr(pos1 - 2 , pos2 - 1);
					var fecha = todo.substr(pos2 + 6,  150);
  	...
  	for (i=0; i<(args.length-2); i+=3) { 
		...
		val=MM_findObj(args[i]);
			...
			if ((val=val.value)!="") {
					...
					num = parseFloat(val);
        				if (isNaN(val)) 
						errors+='El importe debe contener un nï¿½mero.\n';
...
do {  
?>
            <option value="<?php echo $row_cliente['codnum']?>"><?php echo substr($row_cliente['razsoc'],0,22)?><?php echo " "?><?php echo $row_cliente['cuit']?></option>
            <?php
